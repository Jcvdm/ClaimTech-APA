-- Migration to update the claim_instruction_enum type and fix the create_claim_with_vehicle function
-- This addresses the type mismatch error when creating claims with instruction values

-- Step 1: Add 'Assess Only' to the claim_instruction_enum type if it doesn't exist
DO $$
BEGIN
    -- Check if the enum type exists
    IF EXISTS (SELECT 1 FROM pg_type WHERE typname = 'claim_instruction_enum') THEN
        -- Check if the value already exists in the enum
        IF NOT EXISTS (
            SELECT 1 
            FROM pg_enum e 
            JOIN pg_type t ON e.enumtypid = t.oid 
            WHERE t.typname = 'claim_instruction_enum' 
            AND e.enumlabel = 'Assess Only'
        ) THEN
            -- Add the new value to the enum
            EXECUTE 'ALTER TYPE claim_instruction_enum ADD VALUE ''Assess Only''';
        END IF;
    ELSE
        -- If the enum type doesn't exist, create it with all values
        EXECUTE 'CREATE TYPE claim_instruction_enum AS ENUM (''Agree Only'', ''Agree and Authorize'', ''Assess Only'')';
    END IF;
END
$$;

-- Step 2: Update the create_claim_with_vehicle function to properly cast the instruction value
...
'Creates a vehicle and claim in a single transaction, ensuring atomicity and preventing job number collisions. Properly validates and casts the instruction value to the claim_instruction_enum type.';
