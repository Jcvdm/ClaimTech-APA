-- Migration to update the create_claim_with_vehicle function to properly cast status
-- This addresses the type mismatch error when creating claims with status values

-- Update the create_claim_with_vehicle function to properly cast the status value
...
DECLARE
  ...
  v_time_of_loss TIME;
  v_type_of_loss type_of_loss_enum;
  v_status claim_status_enum;
BEGIN
  ...
  -- Validate and convert type_of_loss to enum type
  BEGIN
    IF p_claim_data->>'type_of_loss' IS NOT NULL AND p_claim_data->>'type_of_loss' != '' THEN
      v_type_of_loss := (p_claim_data->>'type_of_loss')::type_of_loss_enum;
    END IF;
  EXCEPTION WHEN invalid_text_representation THEN
    RAISE EXCEPTION 'Invalid type_of_loss value: %. Expected one of: Accident, Theft, Fire, Flood, Hail, Vandalism, Other', p_claim_data->>'type_of_loss';
  END;
  
  -- Validate and convert status to enum type
  BEGIN
    IF p_claim_data->>'status' IS NOT NULL AND p_claim_data->>'status' != '' THEN
      v_status := (p_claim_data->>'status')::claim_status_enum;
    ELSE
      v_status := 'New'::claim_status_enum;
    END IF;
  EXCEPTION WHEN invalid_text_representation THEN
    RAISE EXCEPTION 'Invalid status value: %. Expected one of: New, Appointed, In Progress, Report Sent, Authorized, FRC Requested, FRC Active, FRC Finalized, Canceled', p_claim_data->>'status';
  END;
  ...
  INSERT INTO claims (
    ...
    v_type_of_loss, -- Use the validated enum value
    ...
    v_status, -- Use the validated enum value
    p_user_id,
...
'Creates a vehicle and claim in a single transaction, ensuring atomicity and preventing job number collisions. Properly validates and casts the instruction, time_of_loss, type_of_loss, and status values to their respective types.';
