...
const ClaimListParamsSchema = z.object({
  ...
  _refresh: z.number().optional(), // Internal property to force refresh - ignored in the query
});
...
// Define schema for combined claim and vehicle creation
const ClaimWithVehicleInputSchema = z.object({
  vehicle: VehicleDataSchema,
  claim: z.object({
    client_id: z.string().uuid("Client ID must be a valid UUID"),
    client_reference: z.string().optional().nullable(),
    instruction: z.nativeEnum(ClaimInstruction),
    date_of_loss: z.date({ required_error: "Date of Loss is required." }),
    time_of_loss: z.string().optional().nullable(),
    type_of_loss: TypeOfLossEnum.optional().nullable(),
    accident_description: z.string().optional().nullable(),
    claims_handler_name: z.string().optional().nullable(),
    claims_handler_contact: z.string().optional().nullable(),
    ...
    status: z.nativeEnum(ClaimStatus).default(ClaimStatus.NEW).optional(),
    client_special_instructions: z.string().optional().nullable(),
  }),
});
...
export const claimRouter = createTRPCRouter({
  // Create a claim with vehicle in a single transaction
  createClaimWithVehicle: protectedProcedure
    .input(ClaimWithVehicleInputSchema)
    .output(ClaimWithRelationsOutputSchema)
    .mutation(async ({ ctx, input }) => {
      try {
        console.log("[claimRouter] Executing createClaimWithVehicle procedure...");
        console.log("[claimRouter] Input data:", JSON.stringify(input, null, 2));

        // Call the database function to create the claim and vehicle in a single transaction
        const { data, error } = await ctx.supabase.rpc('create_claim_with_vehicle', {
          p_vehicle_data: input.vehicle,
          p_claim_data: input.claim,
          p_user_id: ctx.user.id
        });

        if (error) {
          console.error("[claimRouter] Error creating claim with vehicle:", error);
          throw new TRPCError({
            code: 'INTERNAL_SERVER_ERROR',
            message: `Failed to create claim with vehicle: ${error.message}`,
            cause: error
          });
        }
        ...
        if (!data) {
          throw new TRPCError({
            code: 'INTERNAL_SERVER_ERROR',
            message: 'Failed to create claim with vehicle: No data returned'
          });
        }

        console.log("[claimRouter] Successfully created claim with vehicle:", data);

        // Fetch the created claim with its relations
        ...
        if (claimError) {
          console.error("[claimRouter] Error fetching created claim:", claimError);
          throw new TRPCError({
            code: 'INTERNAL_SERVER_ERROR',
            message: `Failed to fetch created claim: ${claimError.message}`,
            cause: claimError
          });
        }
        ...
        const mappedItem = {
          ...claimData,
          client: Array.isArray(claimData?.clients) ? claimData.clients[0] ?? null : claimData?.clients ?? null,
          vehicle: Array.isArray(claimData?.vehicles) ? claimData.vehicles[0] ?? null : claimData?.vehicles ?? null,
        };

        return ClaimWithRelationsOutputSchema.parse(mappedItem);
      } catch (error) {
        console.error("[claimRouter] Error in createClaimWithVehicle procedure:", error);
        if (error instanceof TRPCError) throw error;
        throw new TRPCError({
          code: 'INTERNAL_SERVER_ERROR',
          message: `Unexpected error creating claim with vehicle: ${error instanceof Error ? error.message : 'Unknown error'}`,
          cause: error
        });
      }
    }),
  ...
  getCounts: publicProcedure
    ...
    .query(async ({ ctx }) => {
      try {
        ...
        const { data: activeData, error: activeError, count: activeCount } = await ctx.supabase
          .from('claims')
        ...
        const { data: additionalsData, error: additionalsError, count: additionalsCount } = await ctx.supabase
          .from('claims')
          .select('status', { count: 'exact' })
          .eq('has_pending_additionals', true);

        const { data: frcData, error: frcError, count: frcCount } = await ctx.supabase
          .from('claims')
          .select('status', { count: 'exact' })
          .or(`status.in.(${ClaimStatus.FRC_REQUESTED},${ClaimStatus.FRC_ACTIVE})`);

        const { data: finalizedData, error: finalizedError, count: finalizedCount } = await ctx.supabase
          .from('claims')
          .select('status', { count: 'exact' })
          .eq('status', ClaimStatus.FRC_FINALIZED);

        const { data: historyData, error: historyError, count: historyCount } = await ctx.supabase
          .from('claims')
          .select('status', { count: 'exact' })
        ...
        // Use the count property instead of length
        return {
          active: activeCount || 0,
          additionals: additionalsCount || 0,
          frc: frcCount || 0,
