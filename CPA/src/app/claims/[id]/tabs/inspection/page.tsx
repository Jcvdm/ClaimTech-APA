...
export default async function InspectionTabPage({ params }: InspectionTabPageProps) {
  ...
  try {
    ...
    if (!claimData.details && !claimData.summary) {
      console.error(`[InspectionTabPage] No claim data found for ${id}, returning 404.`);
      notFound();
    }

    // Prefetch inspections data
    const inspectionsData = await prefetchInspectionsByClaimServer(id);
    console.log(`[InspectionTabPage] Prefetched ${inspectionsData.length} inspections for claim ${id}`);
